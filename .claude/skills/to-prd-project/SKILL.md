...
This skill writes a PRD and publishes it as a GitHub issue in `Jaywhy-webber/claude-mobile`. It does **not** apply `agent:implement` — labeling happens _after_ sub-issues have been created.
...
- Do **not** add `agent:implement` yourself. The PRD has no sub-issues yet, so labeling it now would land in the wrong place. The user labels once sub-issues are in place.
