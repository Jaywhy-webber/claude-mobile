...
Read every result. Build a mental list of: which modules each prior PRD touches, what friction it addresses, and whether it was merged, closed-without-merge, or still open. **All of these count as "already proposed".**
...
Follow the `/to-prd-project` skill to produce the PRD content.

**Critical override:** do not run `gh issue create`. The workflow publishes the issue itself. Your job is to write the title and body; the workflow handles the GitHub mutation.
...
- **Problem** — the friction in current architecture
- **Solution** — what changes, in plain English
- **Benefits** — framed in terms of **locality** and **leverage**; how tests improve
- **Before / After diagram** — a fenced `mermaid` block showing the shallow → deep transition
...
<output>
{
  ...
  "body": "<the full PRD markdown body>",
  "oneLineSummary": "<one sentence describing the proposal>",
...
- **Read-only everywhere.** No commits, no edits to `CONTEXT.md` / ADRs / source files, no `gh issue create`. The workflow does the publish.
